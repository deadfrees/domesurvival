...
Write-Host "============================================================"
Write-Host "DomeSurvival Joseph Questline V7.3.2 AUTO + PATH"
Write-Host "============================================================"
Write-Host "Current Joseph version: $version"

switch ($version) {
    "7.3.2" {
        Write-Host "[OK] V7.3.2 is already installed." -ForegroundColor Green
        exit 0
    }
    "7.3" { }
    "7.2" {
    ...
    default {
        $first = (Get-Content -LiteralPath $Joseph -TotalCount 10) -join "`n"
        Write-Host "[ERROR] Unknown Joseph script version." -ForegroundColor Red
        Write-Host $first
        throw "Unsupported Joseph source; no automatic overwrite was performed."
    }
}

if ((Get-JosephVersion $Joseph) -ne "7.3") {
    throw "Expected V7.3 before path patch, got: $(Get-JosephVersion $Joseph)"
}

Run-Step "APPLY_JOSEPH_STAGE1_PATH_V7_3_2.ps1" "V7.3 -> V7.3.2 Stage 01 path"

if ((Get-JosephVersion $Joseph) -ne "7.3.2") {
    throw "Unexpected final Joseph version: $(Get-JosephVersion $Joseph)"
}

Write-Host ""
Write-Host "[OK] Joseph Questline V7.3.2 installed." -ForegroundColor Green
Write-Host "Next: .\dev\RUN_DEV_FULL.bat"
Write-Host "In world: /josephscript apply"
Write-Host "Fast stage test: /josephscript nextstage"
exit 0
