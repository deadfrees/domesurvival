...
public final class DomeGeometrySelfTest {
    public static void main(String[] args) {
        ...
        List<PlannedBlock> full = DomeStructurePlanner.planFullV12(spec);
        List<PlannedBlock> fromV1 = DomeStructurePlanner.planV12UpgradeFromV1(spec);
        List<PlannedBlock> fromV11 = DomeStructurePlanner.planV12UpgradeFromV11(spec);
        check(full.size() > shell.size(), "V1.2 full plan should include structure extras");
        check(fromV1.size() > 1_000, "V1 -> V1.2 upgrade unexpectedly small: " + fromV1.size());
        check(fromV11.size() > 1_000, "V1.1 -> V1.2 upgrade unexpectedly small: " + fromV11.size());
        ...
        check(has(full, new BlockPoint(spec.airlockCenterX() - spec.airlockHalfWidth(), spec.foundationMinY(), -590), StructureMaterial.FRAME),
                "airlock must have buried foundation");
        check(has(fromV11, new BlockPoint(spec.legacyAirlockCenterX(), spec.baseY() + 1, -590), StructureMaterial.AIR),
        ...
        System.out.println("plannedV12FullOps=" + full.size());
        System.out.println("plannedV1toV12Ops=" + fromV1.size());
        System.out.println("plannedV11toV12Ops=" + fromV11.size());
        System.out.println("airlockX=" + spec.airlockCenterX());
        System.out.println("airlockZ=" + spec.airlockStartZ() + ".." + spec.airlockEndZ());
        System.out.println("foundationY=" + spec.foundationMinY() + ".." + spec.baseY());
    }
