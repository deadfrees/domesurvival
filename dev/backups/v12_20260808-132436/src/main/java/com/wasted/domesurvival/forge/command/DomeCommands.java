...
public final class DomeCommands {
    ...
    private static int generate(CommandSourceStack source) {
        ...
        switch (result) {
            case STARTED -> source.sendSuccess(() -> Component.literal(
                    "Dome V1.1 generation started: " + DomeGenerationService.total() + " block operations queued."), true);
            case ALREADY_RUNNING -> source.sendFailure(Component.literal("Dome generation/update is already running."));
            case ALREADY_GENERATED -> source.sendFailure(Component.literal(
                    "Dome already exists. For the old prototype use /dome upgrade."));
    ...
    private static int upgrade(CommandSourceStack source) {
        ...
        switch (result) {
            case STARTED -> source.sendSuccess(() -> Component.literal(
                    "Dome V1 -> V1.1 upgrade started: " + DomeGenerationService.total() + " block operations queued."), true);
            case ALREADY_RUNNING -> source.sendFailure(Component.literal("Dome generation/update is already running."));
            case NOT_GENERATED -> source.sendFailure(Component.literal("No generated dome found. Use /dome generate."));
            case UP_TO_DATE -> source.sendSuccess(() -> Component.literal("Dome structure is already V1.1/current."), false);
    ...
    private static int check(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal(String.format(
                "Dome zone @ %.1f %.1f %.1f: %s, safe=%s",
                pos.x, pos.y, pos.z, zone.name(), zone.isSafe())), false);
        return 1;
    }
