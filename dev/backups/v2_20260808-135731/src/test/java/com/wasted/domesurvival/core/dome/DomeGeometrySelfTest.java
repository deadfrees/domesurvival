...
public final class DomeGeometrySelfTest {
    public static void main(String[] args) {
        ...
        List<PlannedBlock> full = DomeStructurePlanner.planFullV14(spec);
        List<PlannedBlock> fromV1 = DomeStructurePlanner.planV14UpgradeFromV1(spec);
        List<PlannedBlock> fromV11 = DomeStructurePlanner.planV14UpgradeFromV11(spec);
        List<PlannedBlock> fromV12 = DomeStructurePlanner.planV14UpgradeFromV12(spec);
        List<PlannedBlock> fromV13 = DomeStructurePlanner.planV14UpgradeFromV13(spec);
        check(full.size() > shell.size(), "V1.4 full plan should include structure extras");
        check(fromV1.size() > 1_000, "V1 -> V1.4 upgrade unexpectedly small: " + fromV1.size());
        check(fromV11.size() > 1_000, "V1.1 -> V1.4 upgrade unexpectedly small: " + fromV11.size());
        ...
        check(has(full, new BlockPoint(spec.centerX() + spec.surfaceRadius(), spec.foundationTopY(), spec.centerZ()), StructureMaterial.FOUNDATION),
                "foundation outer edge must sit directly under the shell at R=50");
        check(has(full, new BlockPoint(spec.centerX() + spec.surfaceRadius() - 3, spec.foundationTopY(), spec.centerZ()), StructureMaterial.FOUNDATION),
                "foundation must extend inward into the dome");
        check(!has(full, new BlockPoint(spec.centerX() + spec.surfaceRadius() + 1, spec.foundationTopY(), spec.centerZ()), StructureMaterial.FOUNDATION),
                "current foundation must never extend outside R=50");
        check(has(fromV12, new BlockPoint(spec.centerX() + spec.surfaceRadius(), spec.foundationTopY(), spec.centerZ()), StructureMaterial.FOUNDATION),
                "V1.2 -> V1.4 upgrade must add inward foundation");
        check(has(fromV13, new BlockPoint(spec.centerX() + spec.surfaceRadius() + 1, spec.foundationTopY(), spec.centerZ()), StructureMaterial.CLEAR_LEGACY_FOUNDATION),
                "V1.3 -> V1.4 must remove obsolete exterior plinth");
        check(has(fromV13, new BlockPoint(spec.centerX() + spec.surfaceRadius() - 3, spec.foundationTopY(), spec.centerZ()), StructureMaterial.FOUNDATION),
                "V1.3 -> V1.4 must keep/add inward foundation");
        check(has(fromV12, new BlockPoint(spec.airlockCenterX() - spec.airlockHalfWidth(), spec.baseY(), -589), StructureMaterial.FOUNDATION),
                "V1.2 -> V1.4 upgrade must add visible airlock foundation");
        ...
        System.out.println("plannedV14FullOps=" + full.size());
        System.out.println("plannedV1toV14Ops=" + fromV1.size());
        System.out.println("plannedV11toV14Ops=" + fromV11.size());
        System.out.println("plannedV12toV14Ops=" + fromV12.size());
        System.out.println("plannedV13toV14Ops=" + fromV13.size());
