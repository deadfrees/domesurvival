...
public final class OxygenRules {
    ...
    /** After reserve reaches zero, deal damage every N oxygen updates. */
    public static final int DAMAGE_EVERY_EMPTY_UPDATES = 2;

    /** Minecraft damage points; 2.0 = one heart. */
    ...
    public static StepResult step(int oxygen, int maxOxygen, int emptyUpdates, boolean breathable) {
        ...
        if (current > 0) {
            ...
            // Reaching zero starts a grace interval; damage begins on later updates.
            return new StepResult(next, 0, false);
        }
