...
$GeneratorVersion = '6.8-jarjar-aware-bridge'
...
if (-not (Test-Path -LiteralPath (Join-Path $BridgeClassDir 'MixinSrgBridge.class'))) {
    Write-Host '[ERROR] V6.8 MixinSrgBridge.class is missing.' -ForegroundColor Red
    exit 2
}
...
if ($cacheReady) {
    ...
    if ($bridge -notmatch 'JarJar-aware Mixin SRG Bridge V6\.8' -or
        ...
        $bridge -notmatch 'ACCESSOR\s+getF_62776_\s+->\s+getLevel') {
        $cacheReady = $false
    }
}

if ($cacheReady) {
    Write-Host '[OK] FULL DEV V6.8 JarJar-aware cache is current.' -ForegroundColor Green
    exit 0
}

Write-Host '[CACHE] Rebuilding FULL DEV V6.8 from untouched production JARs...' -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host '[ERROR] V6.7 Scoped Mixin Bridge failed.' -ForegroundColor Red
    exit 10
}
...
$gradleText = @"
// AUTO-GENERATED BY DomeSurvival FULL DEV V6.7
...
Write-Host ''
Write-Host '[OK] FULL DEV V6.8 JarJar-aware cache rebuilt.' -ForegroundColor Green
