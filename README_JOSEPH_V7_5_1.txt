DomeSurvival Joseph V7.5.1 — Progression Balance
================================================
...
V7.5.1 installer correction
----------------------------
V7.5 failed before writing the modified script because its installer assumed
that openMain(player) was immediately followed by a function named addNotice.
The current Joseph script has a different next top-level function.

V7.5.1 finds openMain(player) and replaces it only up to the next top-level JS
function, regardless of that function's name. No quest progress reset is used.
Because V7.5 failed before WriteAllText(), it is safe to apply V7.5.1 directly
over the current project.

Install:
  .\APPLY_JOSEPH_V7_5_1_PROGRESSION_BALANCE.bat
