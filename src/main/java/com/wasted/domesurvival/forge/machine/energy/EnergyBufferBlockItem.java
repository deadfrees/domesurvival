...
public final class EnergyBufferBlockItem extends BlockItem {
    private static final int ENCHANTMENT_VALUE = 8;
    ...
    public InteractionResult place(BlockPlaceContext context) {
        int capacityLevel = EnergyBufferCapacity.getLevel(context.getItemInHand());
        InteractionResult result = super.place(context);

        if (result.consumesAction() && !context.getLevel().isClientSide && capacityLevel > 0) {
            BlockEntity blockEntity = context.getLevel().getBlockEntity(context.getClickedPos());
            if (blockEntity instanceof CapacityEnchantedEnergyBuffer capacityBuffer) {
                capacityBuffer.setCapacityEnchantLevel(capacityLevel);
            }
        }
        return result;
    }
